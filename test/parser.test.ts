...
describe('parseSchema - a table node-sql-parser rejects is read like any other', () => {
  ...
  it('throws, naming the table, when its columns are not all listed in it', () => {
    ...
    expect(() => parseSchema('CREATE TABLE c LIKE p;', { database: 'MySQL' })).toThrow(/Cannot parse the table c: its columns/);
    expect(() => parseSchema('CREATE TABLE c OF mytype;')).toThrow(/Cannot parse the table c: its columns/);
    expect(() => parseSchema('CREATE TABLE c AS SELECT 1 AS a;')).toThrow(/Cannot parse the table c: its columns/);
    expect(() => parseSchema('CREATE TABLE c AS SELECT 1 AS a;', { database: 'SQLite' })).toThrow(/Cannot parse the table c: its columns/);
  });

  it('reads the NOT NULL the database implies: identity and SERIAL columns, and MySQL AUTO_INCREMENT', () => {
    const nullable = (sql: string, database: 'PostgreSQL' | 'MySQL' | 'SQLite' = 'PostgreSQL') =>
      parseSchema(sql, { database })[0].columns.map((c) => [c.name, c.isNullable]);
    expect(
      nullable(`CREATE TABLE t (
        a bigint GENERATED ALWAYS AS IDENTITY,
        b bigint GENERATED BY DEFAULT AS IDENTITY,
        c serial, d bigserial, e smallserial, f serial4,
        g integer
      );`),
    ).toEqual([['a', false], ['b', false], ['c', false], ['d', false], ['e', false], ['f', false], ['g', true]]);
    expect(nullable('CREATE TABLE t (a SERIAL, b int AUTO_INCREMENT, c int, UNIQUE KEY (b));', 'MySQL')).toEqual([
      ['a', false],
      ['b', false],
      ['c', true],
    ]);
    expect(nullable('CREATE TABLE t (a SERIAL, b INTEGER);', 'SQLite')).toEqual([
      ['a', true],
      ['b', true],
    ]);
  });
